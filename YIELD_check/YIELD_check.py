...
fig.savefig(f"{output_png_dir}/YIELD_vs_run/yield_vs_run_{selected_run_type}_{selected_beam_pass}pass_{target_abbrev}.png", bbox_inches = "tight")
...
ax_top.errorbar(df_out.loc[elec_mask, "ibeam"].to_numpy(), df_out.loc[elec_mask, "yield"].to_numpy(), yerr = df_out.loc[elec_mask, "yield_err"].to_numpy(), fmt = "o", markersize = 3, color = "navy", label = "Electrons", zorder = 2)
...
ax_bot.errorbar(df_out.loc[pos_mask, "ibeam"].to_numpy(), df_out.loc[pos_mask, "yield"].to_numpy(), yerr = df_out.loc[pos_mask, "yield_err"].to_numpy(), fmt = "o", markersize = 3, color = "red", label = "Positrons", zorder=2)
ax_bot.tick_params(axis="x")
...
# pdf.savefig(fig)
fig.savefig(f"{output_png_dir}/YIELD_vs_current/yield_vs_current_{selected_run_type}_{selected_beam_pass}pass_{target_abbrev}.png", bbox_inches = "tight")
plt.close(fig)

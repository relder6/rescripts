...
s5 = np.sum(x_fit**5 * dx)

total_integral = np.sum(y_fit * dx)

def poly5_fit_constr(x, b, c, d, e, f):
    a = (total_integral - b*s1 - c*s2 -d*s3 -e*s4 -f*s5) / s0
    return a + b * x + c * x**2 + d * x**3 + e * x**4 + f * x**5

try:
    popt_free, pcov = curve_fit(poly5_fit_constr, x_fit, y_fit, sigma=sigma_fit, absolute_sigma = True)
    b, c, d, e, f = popt_free
    a = (total_integral - b*s1 - c*s2 - d*s3 - e*s4 - f*s5) / s0
    popt = np.array([a, b, c, d, e, f])
    p5_err = np.sqrt(np.diag(pcov))
except Exception as e:
    print(f"Fit failed for {csv_file}: {e}")
    popt_free = np.full(5, np.nan)
    popt = np.full(6, np.nan)
    p5_err = np.full(6, np.nan)

y_model = poly5_fit_constr(x_fit, *popt_free)
chi2 = np.sum(((y_fit - y_model)/sigma_fit)**2)
ndf = len(x_fit) - len(popt_free)
chi2_ndf = chi2 / ndf

x_smooth = np.linspace(np.min(x_fit), np.max(x_fit), 200)
y_smooth = poly5_fit_constr(x_smooth, *popt_free)
...
integral_model = np.sum(poly5_fit_constr(x_fit, *popt_free) * dx)

...
errlog_filepath = "ERR_log/FIT_pcal_error_log.txt"
...
# --------------------------------------------------------------------------
for path in glob.glob("COIN/*") + glob.glob("SHMS/*"):
    os.remove(path)
...
with open(errlog_filepath, "w") as outfile:
    ...
    outfile.write("RUN_ALL_pcal.py ERROR LOG\n")
...
tqdm.write(f"Processing complete. Total runs processed successfully: {len(updated_fit_results)}")

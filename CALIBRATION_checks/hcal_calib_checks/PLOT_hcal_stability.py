...
processed_fit_filepath = "DAT/FIT_hcal_results_processed.dat"
...
with open(fit_results_filepath, "r") as infile, open(processed_fit_filepath, "w") as outfile:
    outfile.write("#runnum\tfit_mean\tmean_err\tfit_sigma\tsigma_err\thms_p\truntype\n") # Writing the header
    for line in infile:
...
plt.title(f"HMS Calorimeter Fitted E/p vs Run Number\nSelected Run Types: {selected}", fontsize=15)

...
for col in ["runnum", "hms_p", "fit_mean", "mean_err", "fit_sigma", "sigma_err", "bin_min", "bin_max", "bin_avg", "bin_total"]:
...
type_markers = {"HMSDIS": "o", "PI-SIDIS": "s", "PI+SIDIS": "d"}
...
make_plot(
    df,
    "HMS Calorimeter Stability (All Momentum Settings)",
    os.path.join(outdir, "all_momenta_stability_hcal.png")
)
...
for mom in available_momenta:
    ...
    make_plot(
        momdf,
        f"HMS Calorimeter Stability ({mom:g} GeV)",
        os.path.join(outdir, f"stability_hcal_{mom:g}GeV.png")
    )

...
report_filepath = "/work/hallc/c-rsidis/replay/pass0p1/REPORT_OUTPUT/SHMS/PRODUCTION/replay_shms_coin_production_{runnum}_-1.report"
run_info_filepath = "/w/hallc-scshelf2102/c-rsidis/relder/hallc_replay_rsidis/AUX_FILES/rsidis_bigtable_pass0p1.csv"

output_filepath = "CSVs/shmsdis_check.csv"
...
for line in run_lines:
    ...
    if (run_type == "shmsdis") and runnum not in skip_runnums:
...
with open(output_filepath, "w") as outfile:
    outfile.write("runnum,ebeam_diff,ibeam_diff,targ,type,q2_difference,xbj_difference,shms_p_diff,shms_th_diff,livetime_diff,ps1_diff,ps2_diff\n")
    ...
    for line in filtered_lines:
        ...
        bcm2cutch, ps3, ps4, trackeff, phystriggers, pelreal, ps1, ps2 = "-999", "-999", "-999", "-999", "-999", "-999", "-999", "-999"
        ...
        if os.path.exists(report_file):
            with open(report_file, "r") as rep:
                for rep_line in rep:
                    ...
                    elif rep_line.startswith("pEL_REAL"):
                        m = re.search(r"([\d.]+)\s", rep_line)
                        if m:
        ...
        if ps1_val == -999 or ps2_val == -999:
            ...
            continue
        livetime_unformatted = float(ps) * float(phystriggers) / float(pelreal)
        if livetime_unformatted > 1:
            livetime_unformatted = 1
        livetime = float(livetime_unformatted)
        nu = abs(float(ebeam)) - abs(float(shms_p))
        shms_th_rad = np.deg2rad(float(shms_th))
        Q2 = 4 * abs(float(ebeam)) * abs(float(shms_p)) * (np.sin(shms_th_rad/2))**2
        epsilon = 1 / (1 + 2 * (1 + (nu**2 / Q2)) * np.tan(shms_th_rad/2)**2)
        ...
        bt_ps1 = runinfo_val["bt_ps1"]
        bt_ps2 = runinfo_val["bt_ps2"]
        ...
        try:
            ...
            xbj_diff = None
        try:
            ps1_diff = float(ps1) - float(bt_ps1)
        except:
            ps1_diff = None
        try:
            ps2_diff = float(ps2) - float(bt_ps2)
        except:
            ps2_diff = None
        ...
        csv_line = ",".join(map(str, [runnum,
                                      ...
                                      f"{ps1_diff:.6f}" if ps1_diff is not None else "N/A",
                                      f"{ps2_diff:.6f}" if ps2_diff is not None else "N/A",]))
        outfile.write(csv_line + "\n")

print(f"\n☢️  Wrote {len(filtered_lines)} SHMSDIS runs to {output_filepath}")

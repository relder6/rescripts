...
def draw_group(tree, cards, cname, title):
    ...
    for i, cardnum in enumerate(cards):
        c.cd(i+1)
        hname = f"h_{cardnum}"
        expr = f"P.dc.{cardnum}.time:P.dc.{cardnum}.wirenum >> {hname}(122,-10,110,110,-100,350)"
        tree.Draw(expr, "", "COLZ")
...
def main():
    ...
    # Draw each group
    draw_group(T, xs, "pdc_xplane", "X planes")
    draw_group(T, us, "pdc_uplane", "U planes")
    draw_group(T, vs, "pdc_vplane", "V planes")

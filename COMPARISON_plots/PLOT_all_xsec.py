...
csv_file = "CSVs/RME_xsec_results.csv"
...
targs = ["Aluminum", "Carbon", "Copper", "Deuterium"]
...
for targ in targs:
    targ_mask = data["target"] == targ
    mask = pass_mask & targ_mask
    xsec = data["xsec_exp"][mask] / data["A"][mask]
    xsec_err = data["xsec_exp_err"][mask] / data["A"][mask]
...
ax.set_ylabel(r"$\sigma_A / A$ $(\mu b/GeV/sr)$")
ax.set_title(f"Extracted Cross Sections per Nucleon\n E = {ebeam:.2f} GeV at {theta:.2f}$^\circ$")

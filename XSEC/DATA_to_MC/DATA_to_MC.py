...
if selected_target_shortname in {"ld2", "lh2"}:
    ...
    if selected_target_shortname == 'ld2':
        R_dummy = 1 / 3.7825
    if selected_target_shortname == 'lh2':
        R_dummy = 1 / 3.5274
    ...
    for var in variables:
        ...
        # dummy
        dummy_histo_filepath = f"{dummy_dir}/{selected_type}_{selected_beam_pass}pass_dummy_{var}_histo.csv"
        dummy_err_filepath   = f"{dummy_dir}/{selected_type}_{selected_beam_pass}pass_dummy_{var}_err.csv"
        ...
        # dummy
        total_charge_elec_dummy = df_yield_dummy.loc[elec_mask_dummy, 'charge'].sum()
        total_charge_pos_dummy = df_yield_dummy.loc[pos_mask_dummy, 'charge'].sum()

        charge_norm_elec_dummy = 1000 / total_charge_elec_dummy
        charge_norm_pos_dummy = 1000 / total_charge_pos_dummy
        ...
        # dummy
        Y_elec_dummy = df_yield_dummy.loc[elec_mask_dummy, bin_cols_dummy].astype(float).sum(axis=0).values
        Y_pos_dummy = df_yield_dummy.loc[pos_mask_dummy, bin_cols_dummy].astype(float).sum(axis=0).values
        ...
        # dummy
        E_elec_dummy = np.sqrt(np.sum(df_err_dummy.loc[elec_mask_dummy, bin_cols_dummy].astype(float).values**2, axis=0))
        E_pos_dummy = np.sqrt(np.sum(df_err_dummy.loc[pos_mask_dummy, bin_cols_dummy].astype(float).values**2, axis=0))
